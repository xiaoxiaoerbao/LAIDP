...
public class CommandUtils {
    ...
    public static void runSH_OneCommand(String title, String shFile, String workingDirectory, String logDir,
                             int threadsNum){
        System.out.println(main.utils.DateTime.getDateTimeOfNow());
        long start=System.nanoTime();
        List<String> commandList= main.utils.IOTool.readAllLines(shFile);
        List<File> logFiles= main.utils.IOTool.getLogFile(title, logDir, commandList.size());
        ...
        if (failCommandList.size()==0){
            System.out.println(shFile+" all commands had completed in "+ main.utils.Benchmark.getTimeSpanHours(start)+ " hours");
        }else {
            main.utils.IOTool.writeAllLines(new File(logDir, title+".failedRunCommands.sh"), failCommandList);
            ...
            System.out.println(shFile+" total spend "+ main.utils.Benchmark.getTimeSpanHours(start)+ " hours");
        }
    }
    ...
    public static void runSH_OneCommand(String title, List<String> commandList, String workingDirectory, String logDir,
        ...
        if (failCommandList.size()==0){
            System.out.println(" all commands had completed in "+ main.utils.Benchmark.getTimeSpanHours(start)+ " hours");
        }else {
            main.utils.IOTool.writeAllLines(new File(logDir, title+".failedRunCommands.sh"), failCommandList);
            System.out.println(failCommandList.size()+ "commands run failed");
            System.out.println("Commands run failed had been written to "+logDir);
            System.out.println(" total spend "+ main.utils.Benchmark.getTimeSpanHours(start)+ " hours");
        }
    }
